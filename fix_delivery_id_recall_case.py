...
try:
    ...
    for p_id, sn in products:
        ...
        # 1. 'recall_items'와 'recall_cases'를 조인하여 가장 최근의 리콜 접수일 조회
        cursor.execute("""
            SELECT MAX(rc.receipt_date) 
        ...
        if row_del:
            ...
            # 리콜 접수일이 없거나, 납품일이 리콜 접수일보다 확연히 늦은 경우에만 유효한 납품으로 인정
        ...
        if curr_did != best_did:
            ...
            if sn == 'KT022':
                 print(f"  Fixed KT022: {curr_did} -> {best_did} (Ship: {last_ship_date}, Recall: {last_recall_date})")

    conn.commit()
    print(f"\nCorrection Completed using Recall Cases:")
    print(f"  - Set to NULL (Ready for Shipping): {count_to_null} products")
