...
def call_gemini_with_retry(model, contents, log_callback=None, cancel_check=None, max_retries=3, initial_delay=5):
    """Gemini API 호출 (재시도 로직 포함 - Quota Exceeded 시 즉시 중단)"""
    delay = initial_delay
    for attempt in range(max_retries):
        ...
        try:
            ...
            # 1. Quota Exceeded 또는 429 에러 -> 즉시 중단 (사용자 요청)
            if "429" in err_msg or "quota" in err_msg.lower():
                log_msg = f"⛔ Quota exceeded (무료 사용량 초과). 작업을 중단합니다. - {e}"
                print(log_msg)
                if log_callback: log_callback(log_msg)
                raise Exception("Quota exceeded - Abort") # 즉시 예외 발생시켜 중단
            
            # 2. 기타 에러 -> 재시도
            msg = f"⚠️ API Error: {e}. Retrying in {delay}s... (Attempt {attempt+1}/{max_retries})"
            print(msg)
            if log_callback: log_callback(msg)
            
            # 대기 중에도 취소 체크 (1초 단위로 쪼개서 대기)
...
def analyze_pdf(file_path, progress_callback=None, cancel_check=None, log_callback=None, model_name='gemini-2.0-flash'):
    """
    PDF 파일을 분석하여 QC 결과를 반환합니다.
    :param file_path: 분석할 PDF 파일 경로
    :param progress_callback: 진행률 콜백 (msg, percent)
    :param cancel_check: 취소 여부 확인 콜백 (True 반환 시 중단)
    :param log_callback: 로그 메시지 콜백 (msg)
    ...
    log(f"🔄 분석 시작: {file_path} (Model: {model_name})")

    if not os.path.exists(file_path):
        log("❌ 오류: 파일을 찾을 수 없습니다.")
        return []
    ...
    try:
        ...
        # [Page 1 Analysis]
        if cancel_check and cancel_check(): return None
        if progress_callback: progress_callback("Page 1 분석 중... (기본 정보 및 육안 검사)", 20)
        ...
        """
        log("🤖 Page 1 분석 중...")
        res_p1 = call_gemini_with_retry(model, [sample_file, prompt_p1], log_callback=log_callback, cancel_check=cancel_check)
        if cancel_check and cancel_check(): return None
        time.sleep(3) # Rate Limit 방지 (6s -> 3s 단축)

        # [Page 2 Analysis]
        if cancel_check and cancel_check(): return None
        if progress_callback: progress_callback("Page 2 분석 중... (저항, LED, 정밀 측정)", 40)
        prompt_p2 = """
        ...
        """
        log("🤖 Page 2 분석 중...")
        res_p2 = call_gemini_with_retry(model, [sample_file, prompt_p2], log_callback=log_callback, cancel_check=cancel_check)
        if cancel_check and cancel_check(): return None
        time.sleep(3)

        # [Page 3 Analysis]
        if cancel_check and cancel_check(): return None
        if progress_callback: progress_callback("Page 3 분석 중... (Voltage Monitors, 파형 J6A)", 60)
        prompt_p3 = """
        ...
        """
        log("🤖 Page 3 분석 중 (Voltage Monitors, J6A)...")
        res_p3 = call_gemini_with_retry(model, [sample_file, prompt_p3], log_callback=log_callback, cancel_check=cancel_check)
        if cancel_check and cancel_check(): return None
        time.sleep(3)

        # [Page 4 Analysis]
        if cancel_check and cancel_check(): return None
        if progress_callback: progress_callback("Page 4 분석 중... (파형 J6B, J7A)", 80)
        prompt_p4 = """
        ...
        """
        log("🤖 Page 4 분석 중 (J6B, J7A)...")
        res_p4 = call_gemini_with_retry(model, [sample_file, prompt_p4], log_callback=log_callback, cancel_check=cancel_check)
        if cancel_check and cancel_check(): return None
        time.sleep(3)

        # [Page 5 Analysis]
        if cancel_check and cancel_check(): return None
        if progress_callback: progress_callback("Page 5 분석 중... (파형 J7B, J9A)", 90)
        prompt_p5 = """
        ...
        """
        log("🤖 Page 5 분석 중 (J7B, J9A)...")
        res_p5 = call_gemini_with_retry(model, [sample_file, prompt_p5], log_callback=log_callback, cancel_check=cancel_check)
        if cancel_check and cancel_check(): return None
        time.sleep(2)
...
def save_to_excel(results, source_pdf_path, output_filename="정밀검사결과.xlsx", log_callback=None):
    def log(msg):
        print(msg)
        if log_callback: log_callback(msg)
...
# ---------------------------------------------------------
def process_waveform(wf_key, title, data_dict, criteria, final_results_list, unit_label="us", log_callback=None):
    def log(msg):
        print(msg)
        if log_callback: log_callback(msg)

    log(f"\n=== {title} ===")
    log(f"{'항목':<12} | {'표값':<8} | {'이미지값(계산)':<15} | {'일치여부'} | {'스펙체크'}")
    log("-" * 100)

    wf_data = data_dict.get(wf_key, {})
    if not wf_data:
        log(f"⚠️ 데이터 없음: {wf_key}")
        return
    ...
    log(f"ℹ️ 이미지 분석: Freq={img_freq_khz}kHz → Period={round(calc_period, 1)}{unit_label}, Vpp={img_vpp}V")
...
if __name__ == "__main__":
    ...
    if data:
        ...
        save_to_excel(final_results, target_file, log_callback=log_callback)
    else:
        print("데이터 추출 실패")